...
def save_sites(sites):
    want_save = input('\n[!] '+lP+str(len(sites)) + ' Sites has been founds, ¿You want save this?(Y/n):: '+lR)

    if not want_save or want_save == 'y'  or want_save == 'Y' or want_save == 'yes' or want_save == 'Yes':
        where_save = input(G+' \n[+] ¿Where you want save sites.txt?(/home/someplace) by default in this folder:: '+lR)
        if not where_save:
            where_save = '.'
        ...
        for site in sites:
            print(lR+'wait......'+b+site+lR+' is vulnerable but not saved.')
